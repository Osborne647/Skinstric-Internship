...
export default function SexPage() {
  ...
  const [selectedStat, setSelectedStat] = useState("sex");
  ...
  const confidence = getTopConfidence(data.gender);

  const activeGender = selectedGender || confirmed.gender || topGender;
  const activeConfidence = data.gender?.[activeGender]
    ? Math.round(data.gender[activeGender] * 100)
    : confidence;
    ...
    <section className={styles.page}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.confidenceCircle}>
          <span className={styles.selectedLabel}>{activeGender}</span>
        ...
        <div className={styles.rightPanel}>
          ...
          <div className={styles.raceList}>
            {getSortedEntries(data.gender).map(([gender, score]) => (
              <div
                key={gender}
                className={`${styles.raceRow} ${selectedGender === gender ? styles.raceRowActive : ""}`}
              ...
              >
                ...
                <span className={styles.raceName}>{gender}</span>
                <span className={`${styles.raceScore} ${selectedGender === gender ? styles.raceScoreActive : ""}`}>
